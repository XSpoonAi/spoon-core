                    ...
                    if hasattr(self, 'middleware') and self.middleware:
                        for mw in self.middleware:
                            if hasattr(mw, 'subagents') and mw.subagents:
                                # Subagent execution: each subagent step can take 60s+
                                # Total = subagent_max_steps * step_time * safety_factor
                                max_subagent_steps = max((getattr(s, 'max_steps', 5) or 5 for s in mw.subagents), default=5)
                                estimated_time = max_subagent_steps * 120 * 2  # Double buffer for nested operations
                                step_timeout = max(step_timeout, estimated_time)
                                logger.info(f"Subagent detected: step_timeout={step_timeout}s (subagent max_steps={max_subagent_steps})")
                                break
